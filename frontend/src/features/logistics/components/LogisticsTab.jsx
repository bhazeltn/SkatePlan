...
export function LogisticsTab({ team, skater, isSynchro, readOnly }) {
  ...
  // If parent (and not already provided skater), find their skater
  useEffect(() => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ARCHIVED TRIPS (Only visible if NOT readOnly, i.e., Coach/Owner) */}
...
function InfoBlock({ icon: Icon, label, text, placeholder }) {
    return (<div className="space-y-2"><h4 className="text-xs font-bold uppercase text-gray-500 flex items-center gap-2"><Icon className="h-4 w-4 text-gray-400" /> {label}</h4><p className="text-sm text-gray-800 whitespace-pre-wrap leading-relaxed">{text || <span className="text-gray-400 italic">{placeholder}</span>}</p></div>);
}
function TravelSummary({ segments }) {
    ...
    return (<div className="space-y-2">{segments.map((seg, i) => (<div key={i} className="flex items-center gap-2 text-sm text-gray-800 bg-white p-2 rounded border shadow-sm"><span className="font-bold text-[10px] bg-slate-100 px-1.5 py-0.5 rounded uppercase tracking-wide text-slate-600">{seg.type}</span><div className="flex-1 min-w-0 truncate"><span className="font-medium mr-2">{seg.carrier} {seg.number}</span>{seg.dep_time && <span className="text-xs text-gray-500">({seg.dep_time} - {seg.arr_time})</span>}</div></div>))}</div>);
}