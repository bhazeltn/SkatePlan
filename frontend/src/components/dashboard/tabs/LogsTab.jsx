...
export function LogsTab({ skater, team }) {
  const { token } = useAuth();
  ...
  const fetchUrl = team 
      ? `/teams/${team.id}/logs/` 
      : (skater ? `/skaters/${skater.id}/logs/` : null);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <LogSessionModal skater={skater} team={team} onLogCreated={fetchLogs} />
      </div>
                        ...
                        <div className="flex flex-col md:flex-row justify-between gap-4">
                            <div className="min-w-[200px] space-y-2">
                                ...
                                <div className="flex gap-0.5" title="Session Quality">
                                    {[1, 2, 3, 4, 5].map((star) => (
                                        <Star key={star} className={`h-4 w-4 ${star <= (log.session_rating || 0) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-200'}`} />
                                    ))}
                                </div>
                            ...
                            <div className="min-w-[180px] p-2 bg-slate-50 rounded border text-sm flex flex-col gap-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex gap-0.5">
                                        {[1, 2, 3, 4, 5].map((val) => (<Zap key={val} className={`h-3 w-3 ${val <= (log.energy_stamina || 0) ? 'fill-blue-400 text-blue-400' : 'text-gray-300'}`} />))}
                                    </div>
                                </div>
