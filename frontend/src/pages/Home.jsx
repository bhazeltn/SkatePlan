...
import { 
    ...
    ShieldCheck, ClipboardList, Settings, Plus, Users, UserPlus, Handshake
} from 'lucide-react';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            <div className="space-y-6">
                ...
                <Card className={stats.red_flags?.planning?.length > 0 ? "border-orange-200 bg-orange-50/30" : "border-slate-200 bg-slate-50/50"}>
                    ...
                    <CardContent>
                        {stats.red_flags?.planning?.length === 0 ? (<div className="flex items-center gap-2 text-sm text-slate-600"><CheckCircle2 className="h-4 w-4 text-slate-400" /> All active skaters planned.</div>) : (
                            <div className="space-y-2">{stats.red_flags?.planning?.map((item, i) => (<div key={i} className="text-sm text-orange-900 bg-white border border-orange-100 px-3 py-2 rounded shadow-sm"><span className="font-bold">{item.skater}</span><SharedBadge isShared={item.is_shared}/>: {item.issue}</div>))}</div>
                        )}
                    </CardContent>
                </Card>
